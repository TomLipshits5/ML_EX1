...
def predictAndCalcError(k: int, x_train: np.array, y_train: np.array, x_test: np.array, y_test: np.array):
    ...
    print('nn.learnknn: {0}'.format(current_milli_time() - startTime))
    startTime = current_milli_time()
...
def runTestCase(x_test: np.array, y_test: np.array, reps: int, corrupt: bool, Xs: list, Ks: list,
                sampleSizes: list, title: str, xlable: str, ylable: str):
...
def task2a():
    ...
    runTestCase(x_test, y_test, 10, False, sampleSizes, Ks, sampleSizes, "Task 2a", "train sample size", "error")
...
def task2e():
    ...
    x_test, y_test = getTestSample(len(test2) + len(test3) + len(test5) + len(test6))
    # '''
    runTestCase(x_test, y_test, 10, False, Ks, Ks, sampleSizes, "Task 2e", "K value", "error")
...
def task2f():
    ...
    x_test, y_test = getTestSample(len(test2) + len(test3) + len(test5) + len(test6))
    # '''
    runTestCase(x_test, y_test, 10, True, Ks, Ks, sampleSizes, "Task 2f", "K value", "error")
